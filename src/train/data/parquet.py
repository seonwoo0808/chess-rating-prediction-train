...
def projected_columns(source):
    """Use physical nested paths (including LIST wrappers) to read moves and optional clocks."""
    ...
    if movements:
        clocks = [path for path in paths if path.endswith(".time")]
        return ["white_elo", "black_elo", *movements, *clocks]
...
def arrow_numpy_columns(batch):
    """Return flat moves, per-game offsets/validity, numeric ratings, and flat [seconds, present] clocks."""
    ...
    clocks = np.zeros((len(children), 2), dtype=np.float32)
    if pa.types.is_struct(children.type):
        ...
        if children.type.get_field_index("time") >= 0:
            times = children.field("time")
            if not (pa.types.is_integer(times.type) or pa.types.is_floating(times.type)
                    or pa.types.is_null(times.type)):
                raise TypeError("ply_list.time must contain numeric seconds")
            observed = times.is_valid().to_numpy(zero_copy_only=False)
            if observed.any():
                seconds = times.fill_null(0).to_numpy(zero_copy_only=False)
                if np.any(~np.isfinite(seconds)) or np.any(seconds < 0):
                    raise ValueError("ply_list.time must contain finite non-negative seconds")
                clocks[:, 0] = seconds
                if not np.isfinite(clocks[:, 0]).all():
                    raise ValueError("ply_list.time exceeds float32 range")
                clocks[:, 1] = observed
    ...
    return moves, offsets, present, rating("white_elo"), rating("black_elo"), clocks
    ...
    with pq.ParquetFile(path) as parquet_file:
        ...
        for group in range(parquet_file.num_row_groups):
            ...
            while True:
                ...
                if left < right:
                    moves, offsets, present, white, black, clocks = arrow_numpy_columns(
                        batch.slice(left, right-left))
                    yield moves, offsets, present, white, black, clocks
                if offset >= stop:
                    return
            group_start = group_end
