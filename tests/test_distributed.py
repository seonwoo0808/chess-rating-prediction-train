...
class SmallModel(torch.nn.Module):
    ...
    def forward(self, boards, valid, clocks=None):
...
def batches(n, rating=1860.0):
    return [((torch.ones(n, 2), torch.ones(n, 1, dtype=torch.bool), torch.zeros(n, 1, 2)),
...
def write_games(path, start, count):
    pq.write_table(pa.table({
        ...
        "ply_list": pa.array([[{"movement": 12 | (28 << 6),
                                 "time": None if i % 3 == 0 else i * 60}]
